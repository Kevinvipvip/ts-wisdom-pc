<template>
  <div class="news-detail page">
    <background type="border">
      ...
      <div class="page-box" v-if="news_list.length>0">

        <p v-if="prev_i === news_list.length-1" @click="prev_pages(news_list[prev_i].id)">上一篇：没有上一篇了</p>
        <p class="one-line-ellipsis" v-else @click="prev_pages(news_list[prev_i].id)">
          上一篇：{{news_list[prev_i].title}}</p>

        <p v-if="next_i===0" @click="next_pages(news_list[next_i].id)">下一篇：没有下一篇了</p>
        <p class="one-line-ellipsis" v-else @click="next_pages(news_list[next_i].id)">
          下一篇：{{news_list[next_i].title}}</p>
      </div>
    </background>
  </div>
</template>
...
<script>
  export default {
    ...
    methods: {
      ...
      prev_pages(id) {
        this.id = id;
        if (this.prev_i !== this.news_list.length - 1) {
      ...
      share(type, title) {
        if (type === 2) {
          ...
          this.qrcode.clear();
          this.qrcode.makeCode(this.config.url + 'wap/#/detail_news?id=' + this.detail.id);
        }
      },
      ...
      // 获取新闻公告数据内容
      getArticleList() {
        this.utils.ajax(this, 'api/articleList', { page: 1, perpage: 1000 }).then(res => {
          this.news_list = res.list;
        }).then(() => {
          let obj = this.news_list;
