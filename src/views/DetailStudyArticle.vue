<template>
  <!--学术资讯详情-->
  <div class="news-detail page">
    <background type="border">
      <crumb :to="from_type === 2?'/study_article':'/academic'" :to_title="from_type===2?'学术资讯':'学术研究'"/>
      <div class="detail-title">{{detail.title}}</div>
      <div class="mini-title">
        <div class="mini-item">
          <div class="icon"><img src="../assets/icon-time.png"/></div>
          <p>发布时间：{{detail.create_time}}</p>
          <div class="icon eye"><img src="../assets/icon-eye.png"/></div>
          <p>阅读量：{{detail.views}}</p>
        </div>
        <div class="mini-item share">
          分享至：
          <div class="icon"><img src="../assets/wechat.png"/></div>
          <div class="icon"><img src="../assets/qq.png"/></div>
          <div class="icon"><img src="../assets/sina.png"/></div>
        </div>
      </div>
      <div class="rich-text" v-html="detail.content"></div>

      <div class="page-box" v-if="article_list.length>0">

        <p v-if="prev_i === article_list.length-1" @click="prev_pages(article_list[prev_i].id)">上一篇：没有上一篇了</p>
        <p class="one-line-ellipsis" v-else @click="prev_pages(article_list[prev_i].id)">
          上一篇：{{article_list[prev_i].title}}</p>

        <p v-if="next_i===0" @click="next_pages(article_list[next_i].id)">下一篇：没有下一篇了</p>
        <p class="one-line-ellipsis" v-else @click="next_pages(article_list[next_i].id)">
          下一篇：{{article_list[next_i].title}}</p>
      </div>
    </background>
  </div>
</template>

<script>
  export default {
    data() {
      return {
        from_type: 0,//来源类型：1为学术研究页；2为学术资讯页

        id: 0,
        prev_i: 0,
        next_i: 2,

        article_list: [],
        detail: {}
      };
    },
    // 跳转本页时参数变化后重新调取新闻公告详情数据
    beforeRouteUpdate(to, from, next) {
      this.id = parseInt(to.query.id);
      this.from_type = parseInt(to.query.from_type);
      this.getStudyArticleDetail();
      next()
    },
    mounted() {
      this.id = parseInt(this.$route.query.id);
      this.from_type = parseInt(this.$route.query.from_type);
      this.getStudyArticleDetail();
    },
    methods: {
      // 上一篇新闻
      prev_pages(id) {
        this.id = id;
        if (this.prev_i !== this.article_list.length - 1) {
          this.$router.replace({ name: 'detail_study_article', query: { id: id, from_type: this.from_type } });
        } else {
          this.$Message.info('没有上一篇了')
        }
      },
      // 下一篇新闻
      next_pages(id) {
        this.id = id;
        if (this.next_i !== 0) {
          this.$router.replace({ name: 'detail_study_article', query: { id: id, from_type: this.from_type } });
        } else {
          this.$Message.info('没有下一篇了')
        }
      },

      // 获取新闻公告详情
      getStudyArticleDetail() {
        let post = { article_id: this.id };
        this.utils.ajax(this, 'study/studyArticleDetail', post).then(detail => {
          detail.create_time = detail.create_time.split(' ')[0];
          this.detail = detail;
          this.getStudyArticleList();
        });
      },
      // 获取新闻公告数据内容
      getStudyArticleList() {
        this.utils.ajax(this, 'study/studyArticleList', { page: 1, perpage: 1000 }).then(res => {
          this.article_list = res.list;
        }).then(() => {
          let obj = this.article_list;
          for (let i = 0; i < obj.length; i++) {
            if (obj[i].id === this.detail.id) {
              if ((i + 1) === obj.length) {
                this.prev_i = i - 1;
                this.next_i = 0;
              } else if (i === 0) {
                this.prev_i = obj.length - 1;
                this.next_i = i + 1;
              } else {
                this.prev_i = i - 1;
                this.next_i = i + 1;
              }
            }
          }
        });
      }
    }
  };
</script>

<style lang="scss" scoped>
  .news-detail {
    .detail-title {
      font-size: 24px;
      color: #333333;
      margin: 39px 0 25px;
    }

    .mini-title {
      display: flex;
      align-items: center;
      justify-content: space-between;

      .mini-item {
        display: flex;
        align-items: center;

        .icon {
          width: 14px;
          height: 14px;
          margin-right: 8px;

          &.eye {
            width: 18px;
            height: 13px;
          }
        }

        p {
          line-height: 14px;
          font-size: 14px;
          color: #999999;
          margin-right: 20px;
        }

        &.share {
          font-size: 14px;
          color: #666666;

          .icon {
            /*cursor: pointer;*/
            width: 28px;
            height: 28px;
            margin-left: 17px;
          }
        }
      }
    }

    .rich-text {
      margin-top: 39px;
      border-top: 1px solid #e8e2ca;
      padding-top: 39px;

      /deep/ img {
        max-width: 100%;
      }
    }

    .page-box {
      margin-top: 67px;
      margin-bottom: 75px;
      display: flex;
      align-items: center;
      justify-content: space-between;

      p {
        cursor: pointer;
        font-size: 18px;
        color: #999999;
        max-width: 25%;

        &:hover {
          color: #d7a35d;
        }
      }
    }
  }
</style>
